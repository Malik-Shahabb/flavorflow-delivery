...
const Navbar = () => {
  ...
  const isCustomer = activeRole === "customer" || !activeRole;
  ...
  const roleBadge = activeRole ? (
    <Badge variant="secondary" className="text-[10px] px-1.5 py-0 font-normal capitalize">
      {activeRole === "owner" ? "Owner" : activeRole === "delivery" ? "Delivery" : "Customer"}
    </Badge>
  ) : null;
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border bg-card/80 backdrop-blur-lg">
      <div className="container flex h-14 items-center justify-between sm:h-16">
        {/* Logo */}
        <Link to={isOwner ? "/owner-dashboard" : isDelivery ? "/delivery-dashboard" : "/"} className="flex items-center gap-2" onClick={closeMobile}>
        ...
        <nav className="hidden items-center gap-6 md:flex">
          ...
          {isOwner && (
            <>
              <Link to="/owner-dashboard" className="text-sm font-medium text-muted-foreground transition-colors hover:text-foreground">Dashboard</Link>
              <Link to="/owner-orders" className="text-sm font-medium text-muted-foreground transition-colors hover:text-foreground">Orders</Link>
              <Link to="/register-restaurant" className="text-sm font-medium text-muted-foreground transition-colors hover:text-foreground">Add Restaurant</Link>
            </>
          )}
          {isDelivery && (
            <>
              <Link to="/delivery-dashboard" className="text-sm font-medium text-muted-foreground transition-colors hover:text-foreground">Dashboard</Link>
            </>
          )}
        </nav>
              ...
              <DropdownMenuContent align="end" className="w-48">
                ...
                {isOwner && (
                  <>
                    ...
                    <DropdownMenuItem onClick={() => navigate("/owner-orders")} className="gap-2 cursor-pointer">
                      <ClipboardList className="h-4 w-4" /> Restaurant Orders
                    </DropdownMenuItem>
                  </>
                )}
          ...
          <nav className="flex flex-col gap-1">
            ...
            {isOwner && (
              <>
                ...
                <Link to="/owner-orders" onClick={closeMobile} className="rounded-lg px-3 py-2.5 text-sm font-medium text-foreground hover:bg-muted flex items-center gap-2">
                  <ClipboardList className="h-4 w-4" /> Orders
                </Link>
                <Link to="/register-restaurant" onClick={closeMobile} className="rounded-lg px-3 py-2.5 text-sm font-medium text-foreground hover:bg-muted flex items-center gap-2">
                  <Store className="h-4 w-4" /> Add Restaurant
                </Link>
              </>
            )}
            {isDelivery && (
              <Link to="/delivery-dashboard" onClick={closeMobile} className="rounded-lg px-3 py-2.5 text-sm font-medium text-foreground hover:bg-muted flex items-center gap-2">
                <Bike className="h-4 w-4" /> Dashboard
              </Link>
            )}
            {user && (
              <>
